...
def main(params):
    ...
    for _ in range(params.max_epoch):
       ...
       # print(scores)
        #print / JSON log
        strr=['V1','V2','V3','V4','V5','V6','V7','V8','V9','V10','V11','V12','V13','V14','V15','V16','V17','V18','V19','V20','V21']
        i=0;
        #TESST_loss=[]
        for k, v in scores.items():
            logger.info("%s -> %.6f" % (k, v))
            chek2 = torch.load('TestLoss')
            TESST_loss= chek2[strr[i]]
            TESST_loss.append(v)
            chek2[strr[i]] = TESST_loss
            torch.save(chek2, 'TestLoss')
            i+=1
        if params.is_master:
            logger.info("__log__:%s" % json.dumps(scores))
...
if __name__ == '__main__':
    ...
    torch.save({'V1': [],'V2': [],'V3': [],'V4': [],'V5': [],'V6': [],'V7': [],'V8': [],'V11': [],'V12': [],'V13': [],'V14': [],'V15': [],'V16': [],'V17': [],'V18': [],'V19': [],'V20': [],'V9': [],'V10': [],'V21':[]}, 'TestLoss')
    torch.save({'MA-en': [],'MA-fr': [],}, 'TrainLoss')
