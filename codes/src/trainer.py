...
class Trainer(object):
    ...
    def print_stats(self):
        ...
        #print('\n ++++++++Ya Zahra')        
        #print(format(k, np.mean(v)) for k, v in self.stats.items()
         #   if type(v) is list and len(v) > 0)
        #print(s_stat)
        #s_stat = 'MA-en: 10.4657 || MA-fr: 10.3732'
        location = ([pos for pos, char in enumerate(s_stat) if char == ':'])
        first = s_stat[location[0]+2:location[0]+9]
        second =  s_stat[location[1]+2:location[1]+9]

        #print('F: ', first, 'S: ', second)
        chek = torch.load('TrainLoss')
        First = chek['MA-en']
        Second = chek['MA-fr']
        first = float(first)
        second = float(second)
